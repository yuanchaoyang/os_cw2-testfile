...
static bool same_info_exact(const struct addr_space_info *a,
                            const struct addr_space_info *b) {
...
static void test_pid0_equals_self(struct test_state *ts) {
    ...
    bool ok = (r1 == 0 && r2 == 0 && same_info_exact(&a, &b));
...
static void test_cross_pid_child(struct test_state *ts) {
    ...
    if (pipe_read_full(c2p[0], &child_self, sizeof(child_self)) == 0) {
        ...
        ok = (ret == 0 && child_self.num_vmas != ULONG_MAX && same_info_exact(&child_self, &parent_view));
        record_result(ts, "query another process by pid", ok, false,
                      "child_vmas=%lu parent_vmas=%lu",
